...
// Approach 3: Modal Dialog with Tabs (with drag-and-drop visual)
export default function CategoryMockup3() {
  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'active' | 'archived'>('active');
  const [activeCategories, setActiveCategories] = useState([
    { name: 'Food', expenseCount: 24 },
    { name: 'Transportation', expenseCount: 18 },
    { name: 'Entertainment', expenseCount: 12 },
    { name: 'Shopping', expenseCount: 15 },
    { name: 'Bills', expenseCount: 8 },
    { name: 'Other', expenseCount: 5 },
  ]);
  const [archivedCategories, setArchivedCategories] = useState([
    { name: 'Pets', expenseCount: 7 },
    { name: 'Healthcare', expenseCount: 3 },
  ]);
  ...
  const handleEdit = (index: number) => {
    setEditingId(index);
    setEditValue(activeCategories[index].name);
  };
  ...
  const handleArchive = (index: number) => {
    ...
    if (confirm(`Archive "${category.name}"? ${category.expenseCount} expenses will remain but category won't be available for new expenses.`)) {
      setArchivedCategories([...archivedCategories, category]);
      setActiveCategories(activeCategories.filter((_, i) => i !== index));
    }
  };

  const handleRestore = (index: number) => {
    const category = archivedCategories[index];
    setActiveCategories([...activeCategories, category]);
    setArchivedCategories(archivedCategories.filter((_, i) => i !== index));
  };

  const handleAdd = () => {
    if (newCategory.trim()) {
      setActiveCategories([...activeCategories, { name: newCategory.trim(), expenseCount: 0 }]);
      setNewCategory('');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Approach 3: Modal Dialog with Tabs</h1>
            <p className="text-slate-400">Focused UI with active/archived separation - click the button →</p>
          </div>
          <button
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
          ...
          <p className="text-slate-500 mt-2">Click "Manage Categories" to open the modal →</p>
        </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {editingId === index ? (
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-slate-400 text-sm">{category.expenseCount} expenses</div>
                              </div>
                            )}
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-slate-400 text-sm">{category.expenseCount} expenses (archived)</div>
                              </div>
                              <button
